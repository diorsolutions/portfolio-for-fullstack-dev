...
export default function Navigation({
  ...
  return (
    <nav
      ...
      } backdrop-blur-md border-b ${isDark ? "border-[#1C2541]" : "border-gray-200"}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo */}
          <div className={`font-bold text-xl ${isDark ? "text-[#6FFFE9]" : "text-[#0B132B]"}`}>diorsolutions</div>
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button
              ...
              className={`md:hidden ${isDark ? "text-[#6FFFE9] hover:bg-[#1C2541]" : "text-[#0B132B] hover:bg-gray-100"}`}
            >
        ...
        {/* Mobile Navigation */}
        {isMenuOpen && (
          <div className={`md:hidden py-4 border-t ${isDark ? "border-[#1C2541]" : "border-gray-200"}`}>
            {sections.map((section) => (
              <button
                key={section}
                onClick={() => scrollToSection(section)}
